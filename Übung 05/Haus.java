...
public class Haus {
	...
	public boolean WohnungenLeer(boolean[] wohnungen, int Anzahlwohnungen) {// boolean[] wohnungen, int
																					// Anzahlwohnungen
		int bewohnt = 0;
		boolean leer = false;
	...
	public static Haus[] Wreckingball (int jahr, Haus[] H) {
		for (int i=0; i < H.length; i++) {
			if (((H[i].WohnungenLeer(H[i].wohnungen, H[i].Anzahlwohnungen))==true)&& (jahr-H[i].Baujahr>=60)&&(H[i].gebaut==true)) {
				H[i].gebaut=false;
			}
		}
		return H;
