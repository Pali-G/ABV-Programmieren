...
public class SimulationStadt {
	...
	public static void printStrassenListe (Stadt stadt, Strasse[] strassen) {
		System.out.println("|Strassennummer\t|"+"Anzahl grundstücke\t|"+"bebaute Grundstücke\t|"+"Anzahl Wohnungen insg.\t|"
				+"bewohnte Wohnungen insg.\t|");
	...
	public static void printHaus (Stadt stadt, Strasse[] strassen, int Stnr, int Hnr) {
		...
		System.out.println("Haus " + Hnr + " wurde im Jahr "
				+ strassen[Stnr - 1].Häuser[Hnr - 1].Baujahr + " gebaut.");
		System.out.println("Es hat " + strassen[Stnr - 1].Häuser[Hnr - 1].AnWohn
				+ " Wohnungen, davon sind " + strassen[Stnr - 1].Häuser[Hnr - 1].wohnungenbewohnt + " bewohnt.");
		}
		else {
	...
	public static void printHausListe (Stadt stadt, Strasse[] strassen, int Stnr) {
		...
		for (int i=0; i < strassen[Stnr-1].Häuser.length; i++) {
			...
			System.out.println("|"+strassen[Stnr - 1].Häuser[i].Hnr+"\t\t|"+strassen[Stnr - 1].Häuser[i].Baujahr+"\t\t|"
					+strassen[Stnr - 1].Häuser[i].AnWohn+"\t\t\t|"+strassen[Stnr - 1].Häuser[i].wohnungenbewohnt+"\t\t\t|");
			}
			else {
		...
		for (int Jahr = Anfangsjahr; Jahr <= Endjahr; Jahr++) {
			...
			if (Aw1.contains("ja") == true) {
				while (Aw1.contains("ja")==true) {
					...
					System.out.println("Willst du eine Liste aller Häuser oder nur 1 bestimmtes Haus betrachten? (ja/nein=1 Haus)");
